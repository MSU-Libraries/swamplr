...
import os
from swamplr_jobs.models import *
import logging
...
class JobQueue():
    """ Database related updates.
        ...
        The eulcom_jobs table is tan important table that holds the job_id which is connected
        to the other tables in the database
    """
    def __init__(self, job_id):
        """Load batch id for particular job and return data.

        args:
            job_id(str): job id from db.
        """
        self.job_id = job_id
    ...
    @staticmethod
    def get_objects_count(path , type):
        """Count the objects of a particular type.

        args:
            path(str): path to files to tally
            type(str): type of the object
        """
        exclude_string=["_TN","_JP2", "_JPG", "_PREVIEW"]
        count= 0
        for root, dirs, files in os.walk(path):
            for f in files:
                filepath = os.path.join(root, f)
                # If file matches file ending and does not contain string to exclude, process file.
                if (f.lower().endswith(type) and
                   all(ext not in filepath for ext in exclude_string )):
                    count += 1
